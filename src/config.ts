...
export class Config {
  ...
  public egAutonomousApiKey: string;
  ...
  constructor() {
    ...
    this.egAutonomousApiKey = process.env.EG_AUTONOMOUS_API_KEY as string;
  ...
  public ensureConfig() {
    if (!this.egAutonomousApiKey) {
      throw new Error(
        "EG_AUTONOMOUS_API_KEY environment variable is missing. Please set it in your .env file.",
      );
    }
  }
}
