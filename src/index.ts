...
import { EGAutonomousAPI } from "./short-creator/libraries/EGAutonomous";
...
async function main() {
  ...
  const egAutonomousApi = new EGAutonomousAPI(config.egAutonomousApiKey);
  ...
  if (!config.runningInDocker) {
    ...
    if (fs.existsSync(config.installationSuccessfulPath)) {
      ...
      try {
        ...
        // skip pexels test for now as it generates a video
